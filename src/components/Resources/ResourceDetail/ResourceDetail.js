  ...
  const fetchResourceDetails = useCallback(async () => {
    ...
    try {
      ...
      if (docSnap.exists()) {
        ...
        // If not in Firestore, try webResources collection
        const webResourceRef = doc(db, "webResources", resourceId);
        const webDocSnap = await getDoc(webResourceRef);
        
        if (webDocSnap.exists()) {
          console.log("Web resource data:", webDocSnap.data());
          setResource({
            ...webDocSnap.data(),
            id: resourceId
          });
        } else {
          // For resources from Google Maps (not stored in Firestore)
          // We need to check if we have it in local storage or session storage
          ...
          if (storedResources) {
            ...
            if (foundResource) {
              ...
              try {
                ...
                console.log("Resource saved to Firestore");
              } catch (saveError) {
                console.warn("Could not save resource to Firestore:", saveError);
                // Continue anyway since we have the resource from session storage
              }
            } else {
              setError("Resource not found in database or session storage.");
            }
          } else {
