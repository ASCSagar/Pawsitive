...
const ResourceList = () => {
  ...
  const fetchWebResources = useCallback(async () => {
    try {
      ...
      console.log(`Found ${resources.length} web resources`);
  ...
  // This effect combines and filters resources whenever either source changes
  useEffect(() => {
    const allResources = [...mapResources, ...webResources];
    console.log(`Combining resources: ${mapResources.length} map resources + ${webResources.length} web resources`);
    ...
    console.log(`After filtering by "${searchTerm}": ${filtered.length} resources`);
  ...
  return (
    <div className={`min-h-screen bg-${themeColor}-50 p-4 sm:p-6 pb-16 sm:pb-6`}>
      <div className="max-w-7xl mx-auto">
        {/* Header with back button */}
        <div className="mb-6">
          <button
            onClick={() => navigate(isDogCategory ? "/dog-resources" : isCatCategory ? "/cat-resources" : "/")}
          ...
          >
            ...
            Back to {isDogCategory ? "Dog" : isCatCategory ? "Cat" : ""} Resources
          </button>
          
          <h1 className={`text-3xl sm:text-4xl font-bold text-${themeColor}-900 mb-2`}>
            {getCategoryName()} Resources
          </h1>
          
          <p className={`text-${themeColor}-700`}>
            Find the best {getCategoryName().toLowerCase()} resources for your pet
          </p>
        </div>

        {/* View mode toggle */}
        <div className={`bg-white rounded-lg shadow-md p-4 mb-6 flex flex-wrap gap-4 justify-between items-center`}>
          <div className="flex space-x-2">
            <button 
              onClick={() => setViewMode('grid')}
              className={`p-2 rounded ${viewMode === 'grid' ? `bg-${themeColor}-100 text-${themeColor}-600` : 'bg-gray-100 text-gray-600'}`}
            >
            ...
            <button 
              ...
              className={`p-2 rounded ${viewMode === 'map' ? `bg-${themeColor}-100 text-${themeColor}-600` : 'bg-gray-100 text-gray-600'}`}
            >
              <svg xmlns="http://www.w3.org/2000/svg" className="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor">
        ...
        {loading || mapLoading ? (
          <div className="flex justify-center items-center py-16">
            ...
            <p className={`ml-4 text-${themeColor}-700`}>Finding pet resources...</p>
          </div>
        ) : (
          <>
            {viewMode === 'map' ? (
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 mt-6">
                {displayedResources.length > 0 ? (
                  ...
                  <div className="col-span-full text-center py-16">
                    <div className="text-6xl mb-4">{isDogCategory ? '🐕' : isCatCategory ? '😿' : '🔍'}</div>
            ...
            {viewMode === 'grid' && displayedResources.length > 0 && totalPages > 1 && (
              <div className={`mt-8 ${isMobile ? 'fixed bottom-0 left-0 right-0 bg-white border-t border-gray-200 p-2 shadow-md' : ''}`}>
                <PaginationControls />
              </div>
            )}
          </>
